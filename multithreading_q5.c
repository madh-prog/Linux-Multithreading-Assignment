 ...
 //Question 5:Write a program that implements threads synchronization using mutex techniques.
...
void* thread_for_mutex(void *arg)
{
	...
	pthread_mutex_lock(&my_mutex); // critical section
	for(i=0; i<100; i++)
		count++;

	pthread_mutex_unlock(&my_mutex);//release
...
int main()
{
	...
	pthread_mutex_init(&my_mutex, NULL);//initialize
